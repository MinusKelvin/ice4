    ...
    pub fn extract(&mut self, board: &Board) {
        ...
        for &piece in &Piece::ALL {
            for unflipped_square in board.pieces(piece) {
                ...
                kras[color as usize] += (get_king_moves(board.king(!color)) & mob).len() as i32;
                self.mobility[piece as usize] += inc * (mob & !board.colors(color)).len() as f32;
            }
        }

        self.king_ring_attacks = ((kras[Color::White as usize].pow(2) + 1) / 2
            - (kras[Color::Black as usize].pow(2) + 1) / 2) as f32;
