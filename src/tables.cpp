...
int16_t DOUBLED_MG[] = {8, -7, 16, 19, 21, 17, -2, 9};
int16_t DOUBLED_EG[] = {28, 19, 12, 6, 5, 12, 18, 33};
int16_t PROTECTED_PAWN_MG[] = {0, 7, 9};
int16_t PROTECTED_PAWN_EG[] = {0, 7, 4};
int16_t PAWN_SHIELD_MG[] = {-9, 6, 11, 16, 18, 15};
int16_t PAWN_SHIELD_EG[] = {18, -14, -4, 3, -21, -93};
...
void init_tables() {
    unpack_full(0, PAWN, "        BXY>8=6/EUKE?A55FNZXRJ:8NWgd]THBZb{woeSI@ i~tk5g        ", 1.055, 15); // average: 53
    unpack_full(1, PAWN, "        ')1751,.# -,,'&%&('%%(,(-.-*,/227B9??=>;]Bk~xtnx        ", 1.327, 90); // average: 115
    unpack_full(0, PASSED_PAWN, "        -:31/,*1,/%%!!,/..# $'57-6//36D=3*63<ELRQL?GY]~V        ", 1.235, -15); // average: 7
    unpack_full(1, PASSED_PAWN, "        #$ !#%,$'-'%$'+(583/,-23CE=5039?VTN;6<GNd~Y506GC        ", 2.071, -6); // average: 30
    unpack_smol(0, KING, "Q?A`6 \"3JOH6vrXD", 1.0, -41); // average: -2
    unpack_smol(1, KING, "&/, 8LI5YbcWZjj_", 1.0, -37); // average: 5
    unpack_half(0, QUEEN, " '),.26245422-10", 1.0, 640, 641, 644, 657); // average: 656
    unpack_half(1, QUEEN, "8.$ ,.+3,CMK2MWb", 1.0, 1271, 1328, 1266, 1332); // average: 1297
    unpack_half(0, ROOK, "139? )11*512('01", 1.0, 279, 311, 284, 323); // average: 294
