    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int depth, int ply) {
        ...
        for (int j = 0; j < mvcount; j++) {
            if (hashmv.from == moves[j].from && hashmv.to == moves[j].to) {
                ...
                int pawn_attacked =
                    board.board[moves[j].to + (board.stm & WHITE ? 11 : -11)] == ((board.stm ^ INVALID) | PAWN) ||
                    board.board[moves[j].to + (board.stm & WHITE ? 9 : -9)] == ((board.stm ^ INVALID) | PAWN);
                ...
                score[j] = board.board[moves[j].to] * 1e5 - board.board[moves[j].from] + 25e5 * !pawn_attacked;
            } else {
