...
struct Searcher {
    ...
    double abort_time;
    int16_t evals[256];
    int16_t history[2][7][2][SQUARE_SPAN];
            ...
            if (moves[i].from) {
                ...
                if (v >= beta) {
                    for (int j = 0; j < i; j++) {
                        ...
                        int16_t& hist = history[board.stm == BLACK][board.board[moves[j].from] & 7][!board.board[moves[j].to]][moves[j].to-A1];
                        int change = depth * depth;
                        hist -= change + change * hist / MAX_HIST;
                    }
                    int16_t& hist = history[board.stm == BLACK][board.board[moves[i].from] & 7][!board.board[moves[i].to]][moves[i].to-A1];
                    int change = depth * depth;
                    hist += change - change * hist / MAX_HIST;
                ...
                for (int j = 0; j < mvcount; j++) {
                    if (hashmv == moves[j]) {
                        ...
                        score[j] = history
                            ...
                            [!board.board[moves[j].to]]
                            [moves[j].to-A1]
                            + CAPTURE_OFFSET
                            + (board.board[moves[j].to] & 7) * 2048;
