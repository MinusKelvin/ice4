...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int depth, int ply) {
        ...
        for (int j = 0; j < mvcount; j++) {
            int victim = board.board[moves[j].to];
            if (hashmv.from == moves[j].from && hashmv.to == moves[j].to) {
                ...
                score[j] = victim * 1e5
                    ...
                    + history[victim][board.board[moves[j].from]][moves[j].to]
                    ...
                    + 2 * !victim * (*conthist_stack[ply + 1])[board.board[moves[j].from]][moves[j].to]
                    ...
                    + 2.2 * !victim * (*conthist_stack[ply])[board.board[moves[j].from]][moves[j].to];
            }
        }
        ...
        for (int i = 0; i < mvcount; i++) {
            ...
            if (v >= beta) {
                int16_t *hist;
                int16_t change = depth < 0 ? 1 : victim ? 32 * depth : depth * depth;
                for (int j = 0; j < i; j++) {
                ...
                hist = &history[victim][board.board[moves[i].from]][moves[i].to];
                *hist += change - change * *hist / MAX_HIST;
                if (!victim) {
