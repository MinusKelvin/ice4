...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int depth, int ply) {
        ...
        evals[ply] = board.eval(mobilities[ply+1] - mobilities[ply] + TEMPO)
            ...
            + corr_hist[board.stm != WHITE][board.nonpawn_hash[1] % CORR_HIST_SIZE] / CORRHIST_NONPAWN_DIV
            + corr_hist[board.stm != WHITE][board.nonpawn_hash[2] % CORR_HIST_SIZE] / CORRHIST_NONPAWN_DIV
        ...
        int quiets_to_check = pv ? -1 : (int)(LMP_FACTOR * (depth*depth + LMP_CONSTANT)) >> !improving;
        int raised_alpha = 0;
        int legals = 0;
        ...
        for (int i = 0; i < mvcount; i++) {
            ...
            if (is_rep) {
                ...
                if (victim) {
                    reduction = (score[i] - victim * 1e5) / -LMR_CAPTHIST;
                }
                if (reduction < 0) {
                    reduction = 0;
                }
