...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int16_t alpha, int16_t beta, int16_t depth, int ply) {
        ...
        int quiets_to_check = DO_LMP && depth > 0 && depth < 5 && !pv ? quiets_to_check_table[depth] / (1 + !improving) : -1;
        ...
        for (int i = 0; i < mvcount; i++) {
            if (moves[i].from) {
                ...
                if (v >= beta) {
                    if (!victim) {
                        int16_t *hist;
// repetition to reduce code size (compressor deduplicates)
#define APPLY_HISTORY_MALUS *hist -= depth * depth + depth * depth * *hist / MAX_HIST
#define APPLY_HISTORY_BONUS *hist += depth * depth - depth * depth * *hist / MAX_HIST
                        for (int j = 0; j < i; j++) {
...
#undef APPLY_HISTORY_MALUS
#undef APPLY_HISTORY_BONUS
                    }
                    break;
                }
            }
