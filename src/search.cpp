...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int depth, int ply) {
        ...
        int quiets_to_check = pv ? -1 : (depth*depth - depth + 3) / (1 + !improving);
        ...
        for (int i = 0; i < mvcount; i++) {
            ...
            if (best > -20000 && pawn_attacked && (board.board[moves[i].from] & 7) > victim + max(0, depth) / 2) {
                continue;
            }
            ...
            if (best > -20000 && !(quiets_to_check -= !victim)) {
                break;
            }
