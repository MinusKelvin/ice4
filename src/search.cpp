...
struct Searcher {
    ...
    int16_t evals[256];
    HTable history;
    HTable pawn_history[PAWN_HIST_SIZE];
        ...
        for (int j = 0; j < mvcount; j++) {
            if (hashmv.from == moves[j].from && hashmv.to == moves[j].to) {
                ...
                score[j] = history[board.board[moves[j].from] - WHITE_PAWN][moves[j].to-A1]
                    + pawn_history[board.pawn_hash % PAWN_HIST_SIZE][board.board[moves[j].from] - WHITE_PAWN][moves[j].to-A1]
                ...
                if (!victim) {
                    ...
                    for (int j = 0; j < i; j++) {
                        ...
                        *hist -= depth * depth + depth * depth * *hist / MAX_HIST;
                        hist = &pawn_history[board.pawn_hash % PAWN_HIST_SIZE][board.board[moves[j].from] - WHITE_PAWN][moves[j].to-A1];
                        *hist -= depth * depth + depth * depth * *hist / MAX_HIST;
                    ...
                    *hist += depth * depth - depth * depth * *hist / MAX_HIST;
                    hist = &pawn_history[board.pawn_hash % PAWN_HIST_SIZE][board.board[moves[i].from] - WHITE_PAWN][moves[i].to-A1];
                    *hist += depth * depth - depth * depth * *hist / MAX_HIST;
