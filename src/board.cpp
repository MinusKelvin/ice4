...
struct Board {
    ...
    uint8_t stm;
    uint8_t phase;
    int16_t accumulator[2][13];

    Board() : zobrist(0), castle_rights{3,3}, ep_square(0), castle1(0), castle2(0), stm(WHITE), phase(24)
    {
    ...
    void edit(int square, int piece) {
        ...
        if (board[square] & WHITE) {
            accumulator[0][12] -= FT[board[square]][square-A1][12];
            accumulator[1][12] -= FT[board[square] ^ INVALID][square-A1][12];
        } else {
            accumulator[0][12] += FT[board[square] ^ INVALID][flipped-A1][12];
            accumulator[1][12] += FT[board[square]][flipped-A1][12];
        }
        board[square] = piece;
        zobrist ^= ZOBRIST_PIECES[board[square]][square-A1];
        for (int i = 0; i < 12; i++) {
    ...
    int eval() {
        ...
        int pst = (accumulator[0][12] * phase + accumulator[1][12] * (24 - phase)) / 24;
        value += black_first ? -pst : pst;
        return value / 64;
    }
} ROOT;
