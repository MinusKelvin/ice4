...
struct Board {
    ...
    void edit(int square, int piece) {
        for (int i = 0; i < 16; i++) {
            acc[0][i] -= FT_WEIGHTS[board[square]][square-A1][i];
            acc[1][i] -= FT_WEIGHTS[board[square] ^ INVALID][FLIP[square]-A1][i];
        }
    ...
    Board() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            edit(A5 + i, EMPTY);
            edit(A6 + i, EMPTY);
            acc[0][2*i] += FT_BIAS[2*i];
            acc[1][2*i] += FT_BIAS[2*i];
            acc[0][2*i+1] += FT_BIAS[2*i+1];
            acc[1][2*i+1] += FT_BIAS[2*i+1];
        }
    }
    ...
    int eval() {
        ...
        int first = stm == BLACK;
        for (int i = 0; i < 16; i++) {
            v += OUT_WEIGHTS[i] * max(acc[first][i], 0);
        }
        for (int i = 0; i < 16; i++) {
            v += OUT_WEIGHTS[i+16] * max(acc[!first][i], 0);
        }
        return v / 40;
    }
} ROOT;
