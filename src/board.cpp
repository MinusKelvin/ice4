...
struct Board {
    ...
    void update_pawn_eval() {
        ...
        for (int dx = -1; dx < 2; dx++) {
            w_shield_pawns += king_sq[0] < A2 && (board[king_sq[0]+dx+10] == WHITE_PAWN || board[king_sq[0]+dx+20] == WHITE_PAWN);
            b_shield_pawns += king_sq[1] > H7 && (board[king_sq[1]+dx-10] == BLACK_PAWN || board[king_sq[1]+dx-20] == BLACK_PAWN);
        }
