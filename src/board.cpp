...
struct Board {
    ...
    uint8_t check;
    int32_t material_acc[MATNET_SIZE];
    int32_t inc_eval;
    int32_t pawn_eval;
    uint64_t zobrist;
    ...
    void edit(int square, int piece) {
        ...
        for (int i = 0; i < MATNET_SIZE; i++) {
            material_acc[i] -= MATERIAL_FT[board[square]][i];
        }
        board[square] = piece;
        zobrist ^= ZOBRIST.pieces[board[square]][square-A1];
        if ((board[square] & 7) == ROOK) {
    ...
    Board() {
        ...
        memcpy(material_acc, MATERIAL_FT_BIAS, sizeof(MATERIAL_FT_BIAS));
        castle_rights[0] = 3;
        castle_rights[1] = 3;
        stm = WHITE;
    ...
    int eval(int stm_eval) {
        ...
        for (int i = 0; i < MATNET_SIZE; i++) {
            material_factor += MATERIAL_OUT[i] * max(material_acc[i], 0);
        }
        ...
        return (
            ...
            (stm_eval + 0x8000 >> 16) * (24 - phase) * clamp(material_factor, 0, 15000) / 10000
        ) / 24;
    }
} ROOT;
