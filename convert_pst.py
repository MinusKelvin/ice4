...
def dump_string(piece_data, stuff, skip_ranks=False):
    smallest = float("inf")
    largest = -smallest
    average = 0
    for i, value in enumerate(piece_data):
        if (i < 8 or i >= 56) and skip_ranks:
            continue
...
dump_string(scaled[0:64], "0, PAWN", True)
...
dump_string(scaled[384:448], "1, PAWN", True)
