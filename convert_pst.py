...
PALETTE_SIZE = 6
def pick(palette, x):
    palette = [p * r for p in palette[:PALETTE_SIZE] for r in palette[PALETTE_SIZE:]]
    i = min(range(PALETTE_SIZE**2), key=lambda i: abs(palette[i] - x))
    return i, palette[i]
...
soln = least_squares(residuals, list(range(PALETTE_SIZE)) + [1] * PALETTE_SIZE)
...
frequencies = [f - f // 64 for f, _ in freq if f % 64 != 0]
...
print(len(amplitudes), len(frequencies))
print(f"amplitudes: \"{amplitudes_string}\"")
print(f"skips: \"{skips_string}\"")
print(f"base palette: {', '.join(map(lambda x: f'{x:.5}', soln.x[:PALETTE_SIZE]))}")
print(f"residual palette: {', '.join(map(lambda x: f'{x:.5}', soln.x[PALETTE_SIZE:]))}")
